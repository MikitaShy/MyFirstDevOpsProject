...
cd MyVagrant-Ansible/; sudo ansible-playbook jenkins.yml; sudo ansible-playbook playbook_centos_install_docker.yml'
sleep 3
...
sudo docker build -t mish0421/nginx ~/docker/Docker_MyStatic_Site/static-site/ ; sudo docker run -dit -p 8888:80 m.shylau/nginx'
elif [[ ( $edit == "d" ) ]]; then
vagrant destroy
else
echo "====Invalid===="
